...
=== Lab 1.1 Logging into the environment
...
* Quay - https://quayecosystem-quay-quay-enterprise.apps.cluster-{GUID}.{GUID}.example.opentlc.com
+
**login:** admin
+
**password:** admin123
...
=== Lab 1.2 Explore the pipeline stages
...
=== Lab 1.3 The image hardening pipeline

. Navigate back to the jenkins user interface https://jenkins-{USERID}.apps.cluster-{GUID}.{GUID}.example.opentlc.com
+
...
+
. Notice that the pipeline did not pass the compliance gate since we commented out the compliance remediaton from the hardening script. Go to the blue ocean view again and look at the vulnerability report under `Artifacts`
+
...
=== Lab 1.4 Exercise a run of the application pipeline
...
+
. Navigate to the SonarQube url https://sonarqube-ocp-workshop.apps.cluster-{GUID}.{GUID}.example.opentlc.com Click on the number link above *Projects Analyzed* section:
+
image:images/lab7.4-sonarqube-projects2.png[]
+
. Find the project prefixed with your userid. Note there isn’t any vulnerabilities and that the unit test code coverage has been recorded. SonarQube also provides code metrics on items including potential bugs and code smells. Click the Project prefixed with your userid and determine where the vulnerabilities are in the source code.
+
image:images/lab7.4-sonarqube-project-link2.png[]
. Navigate to the nexus url http://nexus-ocp-workshop.apps.cluster-{GUID}.{GUID}.example.opentlc.com, click *browse* then *maven-snapshots*
+
...
. Navigate to the quay url http://quayecosystem-quay-quay-enterprise.apps.cluster-{GUID}.{GUID}.example.opentlc.com, login as *admin* with the password *admin123* if you haven't already
+
...
+
. You can also view the deployed application at http://ecommerce-{USERID}-prod.apps.cluster-{GUID}.{GUID}.example.opentlc.com

=== Lab 1.5 Update the base image and source code (OPTIONAL)
...
. Navigate to your source code repository in gogs http://gogs-ocp-workshop.apps.cluster-{GUID}.{GUID}.example.opentlc.com and login if you haven't already (userid / openshift)
+
...
+
. Update the payload with the ecommerce pipeline generic webhook
(e.g. https://api.cluster-{GUID}.{GUID}.example.opentlc.com:6443/apis/build.openshift.io/v1/namespaces/{USERID}/buildconfigs/{USERID}-ecommerce-pipeline/webhooks/{USERID}-ecommerce-pipeline/generic).
you can copy the webhook link from the console under the BuildConfig pipeline to automatically kickoff the pipeline.
+
